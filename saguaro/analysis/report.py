...
class ReportGenerator:
    ...
    def generate(self) -> Dict[str, Any]:
        ...
        verifier = SentinelVerifier(self.root_path, engines=["native", "ruff", "semantic"]) 
        ...
        for v in violations:
            ...
            eng = v.get('engine', 'unknown') # Assuming we add engine field to violation
        ...
        for ep in entry_points:
            ...
            report["sections"]["architecture"]["entry_point_types"][etype] = \
                report["sections"]["architecture"]["entry_point_types"].get(etype, 0) + 1
    ...
    def save_markdown(self, report: Dict[str, Any], output_path: str):
        ...
        if dc['top_candidates']:
            ...
            for c in dc['top_candidates']:
                lines.append(f"- [{c['confidence']:.2f}] `{c['symbol']}` in {os.path.basename(c['file'])}")
        lines.append("")
