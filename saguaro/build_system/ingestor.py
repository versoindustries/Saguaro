...
class BuildTarget:
    ...
    dependencies: List[str] = field(default_factory=list) # List of target names or packages
...
class BuildGraphIngestor:
    ...
    def _parse_cmake(self, path: str):
        try:
            ...
            libs = re.findall(r'add_library\s*\(\s*(\w+)\s+([^)]+)\)', content, re.DOTALL)
            ...
            exes = re.findall(r'add_executable\s*\(\s*(\w+)\s+([^)]+)\)', content, re.DOTALL)
