...
def main():
    ...
    verify_parser.add_argument(
        "--engines",
        help="Comma-separated list of engines (native,ruff,mypy,vulture)",
        default=None,
    )
    verify_parser.add_argument(
    ...
    if args.command == "deadcode":
        ...
        if not eps:
            ...
            for ep in eps:
                ...
                print(
                    f" - [{ep['type']}] {name} ({os.path.relpath(ep['file'], os.getcwd())}:{ep['line']})"
                )
        ...
        if total_files == 0:
            print(
                "WARNING: No code files found. Check your path or .saguaro/config.yaml exclusions."
            )
            sys.exit(0)
        ...
        verifier = SentinelVerifier(
            repo_path=os.path.abspath(args.path), engines=engines
        )
        violations = verifier.verify_all()
        ...
        if args.fix:
            ...
            if remaining_violations:
                print(
                    f"Attempting AI fixes for {len(remaining_violations)} violations via Scribe..."
                )
                try:
                    ...
                    for v in remaining_violations:
                        ...
                        try:
                            ...
                            if report["status"] == "pass":
                                ...
                                print(
                                    f"    Fix failed verification ❌: {report['violations']}"
                                )
                        except Exception as e:
        ...
        if args.format == "json":
            ...
            if violations:
                print(
                    f"Sentinel Validation Failed: {len(violations)} violations found."
                )
                for v in violations:
                    print(
                        f"[{v['severity']}] {v['file']}:{v['line']} - {v['message']} ({v['rule_id']})"
                    )
