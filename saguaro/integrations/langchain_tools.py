...
class SliceInput(BaseModel):
    ...
    depth: int = Field(
        default=2,
        description="Dependency depth to include (1-5)"
    )
...
class VerifyInput(BaseModel):
    ...
    path: str = Field(
        default=".",
        description="Path to verify (file or directory)"
    )
    engines: str = Field(
...
If violations found, fix them before marking task complete."""
    ...
    def _run(
        ...
        try:
            result = subprocess.run(
                [self.venv_path, "verify", path, "--engines", engines, "--format", "json"],
                capture_output=True,
                text=True,
                cwd=self.repo_path,
                timeout=120
    ...
    async def _arun(self, path: str = ".", engines: str = "native,ruff,semantic") -> str:
