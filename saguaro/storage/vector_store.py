...
class VectorStore:
            ...
            try:
                ...
                if vec_array.ndim == 1 and vec_array.shape[0] == self.dim:
                    ...
                    if i < len(self.metadata):
                        ...
                        valid_metadata.append({"name": f"unknown_{i}", "type": "unknown"})
                else:
                    invalid_count += 1
                    logger.debug(f"Filtered vector {i}: shape {vec_array.shape} != expected ({self.dim},)")
    ...
    def query(self, query_vec: np.ndarray, k: int = 5, boost_map: Dict[str, float] = None) -> List[Dict[str, Any]]:
        """
        ...
        if boost_map:
            ...
            for i, meta in enumerate(self.metadata):
                ...
                if name and name in boost_map:
                    scores[i] += boost_map[name] * 0.2  # Weighted boost using 0.2 factor
