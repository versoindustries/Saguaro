...
class CoherenceManager:
    ...
    def calibrate(self, workset_files: List[str]):
        ...
        logger.info(f"Calibrating Coherence Adapter on {len(valid_files)} files ({total_bytes/1024/1024:.2f} MB). Target Budget: {budget}")
        ...
        from saguaro.tokenization.superword_merger import SuperwordMerger, SuperwordMergerConfig, SuperwordEntry
        ...
        logger.info(f"Fused Vocab: {len(baseline_entries)} base + {added_count} repo = {len(final_entries)} total.")
