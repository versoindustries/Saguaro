...
def train_baseline(
    ...
    if curriculum_name:
        ...
        stream_iter = loader.stream_samples(
            curriculum_name, total_limit_bytes=max_bytes
        )
        if has_tqdm:
            stream_iter = tqdm(
                stream_iter,
        ...
        if has_tqdm:
            pbar = tqdm(
                total=max_bytes, unit="B", unit_scale=True, desc="Reading Corpus"
            )
    ...
    controller.train_codebook(
        samples, min_freq=config.min_ngram_freq, progress_callback=train_callback
    )
...
def main():
    ...
    parser.add_argument(
        "--output",
        default="saguaro/artifacts/codebooks/verso_baseline.json",
        help="Output path",
    )
    parser.add_argument(
        "--fast", action="store_true", help="Fast mode (smaller sample)"
    )
